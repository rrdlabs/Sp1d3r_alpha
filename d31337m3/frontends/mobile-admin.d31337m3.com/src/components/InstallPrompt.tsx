  ...
  return (
    <Collapse in={show}>
      <Box sx={{
        ...
        bgcolor: "secondary.main", color: "white", borderRadius: 2, p: 1.5,
        display: "flex", alignItems: "center", gap: 1, boxShadow: 6,
      }}>
        <GetAppIcon sx={{ flexShrink: 0 }} />
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Install Admin Portal</Typography>
          <Typography variant="caption" sx={{ opacity: 0.8 }}>Add to home screen for quick access</Typography>
        </Box>
        <Button variant="contained" size="small" onClick={handleInstall}
          sx={{ bgcolor: "white", color: "secondary.main", fontWeight: 700, flexShrink: 0, textTransform: "none" }}>
          Install
        </Button>
        <IconButton size="small" onClick={handleDismiss} sx={{ color: "white", flexShrink: 0 }}>
